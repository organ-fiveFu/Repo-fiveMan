...
import com.vblessings.nhs.enums.FtpCatalogEnum;
...
//
        // 体检报告上传处理
        if (examArchiveInsertPO.getExamArchiveFile() != null) {
            // 验证文件类型为pdf
            ftpProperties.validFile(examArchiveInsertPO.getExamArchiveFile());
            // 文件上传FTP
            UploadSuccessVO uploadSuccessVO = iFtpService.upload(examArchiveInsertPO.getExamArchiveFile(), FtpCatalogEnum.EXAM.getCatalog());
            busExamArchive.setExamArchiveUrl(uploadSuccessVO.getDocUrl());
            busExamArchive.setExamArchiveName(uploadSuccessVO.getDocName());
        }
        BeanUtil.copyProperties(examArchiveInsertPO,busExamArchive);
