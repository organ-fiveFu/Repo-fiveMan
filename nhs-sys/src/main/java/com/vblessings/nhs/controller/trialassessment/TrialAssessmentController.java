...
public class TrialAssessmentController {
    ...
    @GetMapping("/query")
    public ResultBody<TrialAssessmentQueryVO> queryTrialAssessment(String archiveId){
        log.info("查询试用期评估信息,archiveId:" + archiveId);
        return ResultBody.newSuccessInstance(trialAssessmentService.queryTrialAssessment(archiveId));
    }
    ...
    @GetMapping("/delete")
    public ResultBody<Boolean> deleteTrialAssessment(String archiveId, @ApiIgnore @CurrentUser UserInfoToken userInfoToken){
        log.info("删除试用期评估信息,archiveId:"+ archiveId);
        return ResultBody.newSuccessInstance(trialAssessmentService.deleteTrialAssessment(archiveId, userInfoToken));
    }
}
